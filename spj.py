...
import requests   # <--- required for OMDb poster fetch
...
def _extract_model_classes(news_model):
    """
    Try to extract class list from the loaded news_model (works with raw pipelines,
    sklearn estimators, or our ModelWrapper).
    Returns list of uppercase class names if available, else fallback DEFAULT_CANONICAL_NEWS.
    """
    try:
        # If it's our wrapper returned by load_model_flexible
        if isinstance(news_model, ModelWrapper):
            # attempt raw.classes_ on underlying model/pipeline
            raw = news_model.raw
            if hasattr(raw, "classes_"):
                return [str(x).strip().upper() for x in list(getattr(raw, "classes_"))]
            # if wrapper built a class_map earlier, use values
            if news_model.class_map is not None:
                vals = list(news_model.class_map.values())
                return [str(x).strip().upper() for x in vals]
            # try pipeline inside wrapper
...
def normalize_news_output(raw_label, news_model=None, canonical_list=None):
    """
    Normalize model raw output into canonical uppercase labels.
    Uses: exact match against model classes (preferred), then tries heuristics, then fallback to 'NEWS'.
    """
    ...
    if s == "":
        return "NEWS"
    # canonical list provided or derived from model
    ...
    if s.upper() in canonical_set:
        return s.upper()
    # try common normalizations: title case, lower etc.
    ...
    try:
        ...
        pass
    # final fallback: try uppercase of raw and if it's in canonical_set use it
    up = s.upper()
...
# ---------------- Load models (cached) ----------------
# Spam model: leave as before (pipeline or estimator)
spam_model = load_pipeline(SPAM_MODEL_PATH)
# News model: use flexible loader -> ModelWrapper (robust)
news_model = load_model_flexible(NEWS_MODEL_PATH)
if news_model is not None:
    # defensive: mark as news model so certain callers know to preprocess
    try:
        setattr(news_model, "is_news", True)
    except Exception:
        pass

# Extract canonical classes for UI filters
NEWS_CANONICAL_CLASSES = _extract_model_classes(news_model) if news_model is not None else DEFAULT_CANONICAL_NEWS

# ---------------- Movie model loader (cached) ----------------
@st.cache_resource
def load_movie_models():
    # Uses _cached_load to get caching behaviour consistent with other loaders
    df = None
    vectors = None
    model = None
    try:
...
# ---------------- USER CHOICE ----------------
    ...
    if not st.session_state.get("logged_in") or not st.session_state.get("user"):
        ...
        # <-- REPLACED 2-button layout with 3-button layout including Movie Recommendation -->
...
# ---------------- USER MODEL PAGE ----------------
    ...
    if not st.session_state.get("logged_in") or not st.session_state.get("user"):
        ...
        # If user selected movies, show a dedicated movie UI and skip the multi-tabs flow that applies to spam/news
        if model_selected == "movies":
            # Movie UI (simple, same look and feel)
            st.markdown("<h4>Movie Recommender</h4>", unsafe_allow_html=True)
            ...
            if df_m is None or vecs_m is None or model_m is None:
                ...
                # select movie
                item = st.selectbox(label='Select a movie', options=df_m.name.values, key="movie_select")
                if st.button("Recommend"):
                    st.header("Here are top 5 movie recommendations")
                    try:
                        index = int(df_m[df_m.name == item].index[0])
                        test_vector = vecs_m[index]
                        scores, indexes = model_m.kneighbors([test_vector], n_neighbors=6)
                        rows = df_m.iloc[indexes[0][1:]]  # skip itself

                        cols = st.columns(5)
                        i = 0
                        for n, mid in zip(rows.name.values, rows.movie_id.values):
                            # fetch poster from OMDB
                            try:
                                url = f"http://www.omdbapi.com/?i={mid}&apikey=48972038"
                                resp = requests.get(url, timeout=6).json()
                                purl = resp.get('Poster', 'N/A')
                            except Exception:
                                purl = "N/A"
                            with cols[i]:
                                st.text(n)
                                if purl and purl != 'N/A':
                                    try:
                                        st.image(purl)
                                    except Exception:
            ...
            # bottom navigation for movie page
            st.markdown("---")
                ...
                if model_selected == "spam":
                    ...
                    if st.button("Predict News"):
                        if news_model is None:
                            ...
                            # --- FIXED single news prediction: robust try paths and normalization to canonical labels ---
                            try:
                                pred_candidates = []
                                # try wrapper/pipeline with preprocessing
                                try:
                                ...
                                try:
                                    ...
                                    pass
                                # direct if underlying raw has predict
                                try:
                                ...
                                # Normalize to canonical ALL CAPS
                                ...
                                pred_norm = "NEWS"
                            try:
                                # Save normalized label to DB for consistent downstream use
            ...
            with tabs[1]:
                ...
                # New: unified postfilter selection requested by user:
                if model_selected == "spam":
                    postfilter_choice = st.selectbox("Show only:", ["All","Ham","Spam"], key=f"postfilter_{model_selected}")
                else:
                    # fixed required categories per user request - use actual classes from model
                ...
                if uploaded is not None:
                    ...
                    # Use a form to avoid rerun while predicting
                        ...
                        if chosen_model is None:
                            ...
                            with st.spinner("Predicting..."):
                                try:
                                    ...
                                    # IMPORTANT: try flexible prediction (model may be wrapper or raw)
                                    is_news_flag = (model_selected != "spam")
                                    # Use chunked_predict but let _try_predict try multiple shapes (we won't force preprocessing here)
                                    ...
                                    if model_selected=="spam":
                                        ...
                                        # Normalize via canonical mapping extracted from model
                                    ...
                                    # save to session so user can change filters without re-predicting
                                    st.session_state["last_bulk_predictions"] = df_pref.copy()

                                    # log predictions (first 500). Store normalized to DB for consistency
                    ...
                    if df_predicted is None:
                        ...
                        if model_selected == "spam":
                            ...
                            # postfilter_choice contains one of the fixed categories; exact equality against ALL CAPS normalized 'result'
