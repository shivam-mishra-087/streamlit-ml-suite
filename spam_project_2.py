...
def load_movie_models_wrapper():
    # reuse load_movie_models logic from Project1 style — search for df.pkl, vectors.pkl, model.pkl
    df = None
    vectors = None
    model = None
    try:
...
# ---------------- Movie model loader (cached) using wrapper above ----------------
@st.cache_resource
                ...
                with left:
                    try:
                        ...
                        if os.path.exists(DEFAULT_POSTER):
                            st.image(DEFAULT_POSTER, use_cotainer_width=True)
                with middle:
